...
The `nearest_neighbor()` model predict new samples in test set by defining a model that uses the K most similar data points from the training set. This model is easy to explain as in this case, we can see this model as a model that predict the share of households with the social assistance program by looking at the numbers from "neighboring" villages. In this analysis, we tuned the model to find the best $K$ that minimize rmse. 
...
# 4 Conclusion
Based on our analysis, the random forest model appears to be the most accurate in predicting the percentage of households in a village which receive free health insurance under the national health assistance program. However, based on the rmse this model is actually not predict the test data really well. Future research should try other models and embed new variables that can help predict the test data better. Moreover, using machine learning models to predict social science outcome might also prone to validity problem (both temporal and spatial) therefore, caution needs to be made."  with "Despite this, the model does not actually predict the outcome variable very well due to the high rmse. Future research should try using other models and embed new variables that may help to better predict the test data. Moreover, using supervised learning models to predict social science outcomes may lead to issues of external validity (both temporal and spatial) therefore, caution must be taken in interpreting the model predictions
...
bind_cols(podes_test, predict(knn_fit, new_data = podes_test)) %>%
      rmse(truth = podes_test$perc_hh_sa, estimate = .pred) %>%
  mutate(method = "Nearest Neighbor"),
